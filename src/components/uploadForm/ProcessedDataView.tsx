...
import { ProcessingResult, ProcessedPlayerData, ProcessedClubData, ProcessedDanceAcademyData } from '@src/utils/functions';
...
export const ProcessedDataView: React.FC<ProcessedDataViewProps> = ({ 
  ...
  const getColumnHeaders = () => {
    switch (templateType) {
      case 'player':
        return ['Nombre', 'Apellido', 'Edad', 'Peso', 'Altura', 'Posición', 'Sexo', 'Clase', 'F. Nacimiento', 'Localidad', 'Escuela/Club', 'Contacto', 'Deporte'];
      case 'club':
        return ['Nombre Club', 'Ciudad', 'Dirección', 'Teléfono', 'Email', 'Presidente', 'F. Fundación'];
      case 'danceAcademy':
        return ['Nombre Academia', 'Director', 'Ciudad', 'Dirección', 'Teléfono', 'Email', 'Tipos Danza'];
      default:
        return [];
    }
  };
  ...
  const renderTableRow = (item: any, index: number) => {
    const values = Object.values(item).filter((_, i) => i < Object.keys(item).length - 1); // Excluir rowNumber
    return (
      <tr key={index} className="border-b border-gray-600">
        {values.map((value: any, cellIndex) => (
          <td key={cellIndex} className="px-3 py-2 text-sm text-white truncate max-w-32" title={value?.toString()}>
            {value?.toString() || '-'}
          </td>
        ))}
      </tr>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-300 text-sm">
              {data.validRows} de {data.totalRows} filas válidas
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        ...
        <div className="bg-red-500/20 border border-red-500 rounded-lg p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-red-400 font-medium">{data.errors.length}</span>
          </div>
        ...
        <div className="bg-blue-500/20 border border-blue-500 rounded-lg p-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-blue-400 font-medium">{data.totalRows}</span>
          </div>
          <p className="text-blue-300 text-xs">Total procesado</p>
        </div>
      </div>
        ...
        <details className="mb-4">
          ...
          <div className="mt-2 max-h-40 overflow-y-auto bg-red-500/10 border border-red-500/30 rounded-lg p-3">
            {data.errors.map((error, index) => (
              <div key={index} className="text-red-300 text-sm py-1 border-b border-red-500/20 last:border-b-0">
                <strong>Fila {error.row}</strong> - Campo '{error.field}': {error.message}
                {error.value && <span className="text-red-400"> (Valor: "{error.value}")</span>}
              </div>
            ))}
          </div>
        </details>
          ...
          {totalPages > 1 && (
            <div className="bg-gray-700 px-4 py-3 border-t border-gray-600 flex items-center justify-between">
              <div className="text-sm text-gray-300">
                Mostrando {startIndex + 1} a {Math.min(endIndex, data.data.length)} de {data.data.length} registros
              </div>
              <div className="flex gap-2">
                <button
